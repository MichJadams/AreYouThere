...
function movement(keycode,playerlocation){
    ...
    const deltMovement = 1 
        if(keycode == 87){
            ...
            if(mapOne.mapTwoCollisionHash[+collisionCheckPlayerLocation]){
...
function winCheck(keycode,playerlocation){
    ...
    const mapOne = require('./mapOne')
            ...
            if(mapOne.mapTwoCollisionHash[+collisionCheckPlayerLocation] === 'winning'){
                console.log("won")
                return true
            }
        else{
...
module.exports ={movement, winCheck}